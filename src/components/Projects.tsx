...
const ProjectCard = async ({ link }: { link: string }) => {
  ...
  return (
    <div className="bg-slate-900 rounded-3xl">
      <div className="bg-slate-700 rounded-lg h-1/4 ">
        <h1 className=" text-white text-lg font-bold p-2 text-center font-bold align-items-center">
          {repo.name}
        </h1>
      </div>
      <p className="text-white text-xs p-4 h-fit sm:h-32">{repo.description}</p>
      <div className="pe-4 ps-4">
        <Link href={repo.html_url} target="_blank" className="rounded-full">
          <button className="flex backdrop-blur-lg bg-white/30 text-slate-100 p-2 rounded-full w-full hover:bg-slate-700">
            <svg
...
const Projects = async () => {
  return (
    <div className="flex grid grid-cols-1 p-8 gap-4 md:grid-cols-4">
